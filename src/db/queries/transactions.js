...
export async function createTransaction(
  {
    virtualAccountId, merchantId, amount, direction,
    status = 'success', matched = true, misdirected = false,
    senderName, senderBank, senderAccount,
    nombaSessionId, nambaTxnId, narration, nombaRawPayload,
  },
  client = pool
) {
  const res = await client.query(
    `INSERT INTO transactions
     ...
     RETURNING *`,
    [
      virtualAccountId, merchantId, amount, direction, status, matched, misdirected,
      senderName ?? null, senderBank ?? null, senderAccount ?? null,
      nombaSessionId ?? null, nambaTxnId ?? null, narration ?? null,
...
export async function findTransactions({ merchantId, virtualAccountId, direction, status, matched, page = 1, pageSize = 20 }) {
  const conds = ['t.merchant_id=$1']
  const params = [merchantId]
  let i = 2

  if (virtualAccountId) { conds.push(`t.virtual_account_id=$${i++}`); params.push(virtualAccountId) }
  if (direction)        { conds.push(`t.direction=$${i++}`);           params.push(direction) }
  if (status)           { conds.push(`t.status=$${i++}`);              params.push(status) }
  if (matched !== undefined) { conds.push(`t.matched=$${i++}`);        params.push(matched) }
...
export async function updateTransactionMisdirected(id, matched, misdirected, client = pool) {
  const res = await client.query(
