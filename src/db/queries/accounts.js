...
export async function createAccount(
  { merchantId, customerId, customerName, kycTier, accountRef, nombaAccountNumber, nambaBankName, nambaBankCode, nombaRawResponse },
  client = pool
) {
  const res = await client.query(
    `INSERT INTO virtual_accounts
     ...
     RETURNING *`,
    [merchantId, customerId, customerName, kycTier, accountRef,
     nombaAccountNumber ?? null, nambaBankName ?? null, nambaBankCode ?? null,
...
export async function findAccounts({ merchantId, status, kycTier, search, page = 1, pageSize = 20 }) {
  const conds = ['merchant_id = $1']
  const params = [merchantId]
  let i = 2

  if (status)  { conds.push(`status = $${i++}`);   params.push(status) }
  if (kycTier) { conds.push(`kyc_tier = $${i++}`); params.push(kycTier) }
  if (search)  {
  ...
  const [rows, count] = await Promise.all([
    ...
    pool.query(`SELECT COUNT(*) FROM virtual_accounts WHERE ${where}`, params),
  ])
...
export async function updateAccountName(id, newName, renameHistory, client = pool) {
  const res = await client.query(
