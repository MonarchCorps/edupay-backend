...
export async function createWebhookEvent({ nombaRequestId, eventType, rawPayload }) {
...
export async function findWebhookEventById(id) {
  const res = await pool.query('SELECT * FROM webhook_events WHERE id=$1', [id])
...
export async function findWebhookEvents({ processed, page = 1, pageSize = 20 } = {}) {
  const conds = []
  const params = []
  let i = 1

  if (processed !== undefined) { conds.push(`processed=$${i++}`); params.push(processed) }
