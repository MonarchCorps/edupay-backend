...
import {
  findAccountById, updateAccountStatus, updateAccountName,
  updateAccountBalance, addAuditLog,
} from '../db/queries/accounts.js'
import {
  findTransactionById, updateTransactionMisdirected,
...
export async function renameAccount({ accountId, merchantId, newName }) {
  ...
  try {
    await nombaVA.updateVirtualAccount(account.account_ref, { accountName: newName })
  } catch (err) {
...
export async function resolveMisdirectedPayment({ transactionId, merchantId, action }) {
